...
function chatbox.WrapText(msgData, maxWidth, initWidth)
	...
	local curText = ""
	local spaceWidth = util.GetTextSize(" ", "flChatFont")
	local dashWidth = util.GetTextSize("-", "flChatFont")
	...
	for key, val in ipairs(parsed) do
		if (isstring(val)) then
			...
			for k, v in ipairs(exploded) do
				local w, h = util.GetTextSize(v, "flChatFont")
				...
				-- if it's more
					...
					if (w > maxWidth) then
						...
						for k2, v2 in ipairs(characters) do
							local w, h = util.GetTextSize(v2, "flChatFont")
...
function chatbox.ParseText(messageData)
	...
	if (messageData.drawAvatar and IsValid(messageData.sender)) then
		if (!file.Exists("flavatars/"..messageData.sender:SteamID64()..".jpg", "DATA")) then
			if (!file.Exists("flavatars", "DATA")) then
				file.CreateDir("flavatars")
			end
	...
	if (messageData.prefix) then
		...
		messageData.prefix = fl.lang:TranslateText(messageData.prefix)

		table.insert(parsed[1], messageData.prefixColor)
		table.insert(parsed[1], messageData.prefix)

		local wide = util.GetTextSize(messageData.prefix, "flChatFont")
	...
	if (messageData.isPlayerMessage) then
		...
		if (messageData.suffix) then
			-- Suffix too!
			messageData.suffix = fl.lang:TranslateText(messageData.suffix)
		end

		table.insert(parsed[1], messageData.playerName..(messageData.suffix or ": "))

		local wide = util.GetTextSize(messageData.playerName..(messageData.suffix or ": "), "flChatFont")
		msgWidth = msgWidth + wide
	end
...
function PANEL:Paint(w, h)
	local backColor = fl.settings:GetColor("MenuBackColor")
			...
			for k2, v2 in pairs(v) do
				...
				elseif (!isnumber(v2)) then
					if (IsTime(v2)) then
						...
						local width = util.GetTextSize(time, "flChatFont")
						offX = offX + width + 2
					...
					elseif (IsAvatar(v2)) then
						...
						local width = util.GetTextSize(v2, "flChatFont")
						offX = offX + width
					end
				elseif (isnumber(v2)) then
					offY = v2
	...
	if (chatbox.IsTypingCommand()) then
		...
		if (command and command != "") then
			...
			for k, v in pairs(fl.command.aliases) do
				...
				if (string.utf8sub(k, 1, commandLen) == string.lower(command) and (!splitTable[2] or string.lower(command) == k)) then
					local cmdTable = fl.command:Find(v)
			...
			for k, v in ipairs(commands) do
				cX = 4
				draw.SimpleTextOutlined("/"..v.name, "flChatSyntax", cX, cY, Color(240, 240, 240), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, Color(0, 0, 0))
				local w = util.GetTextSize("/"..v.name, "flChatSyntax")
				cX = cX + w + 8
				draw.SimpleTextOutlined((v.tip or ""), "flChatFont", cX, cY + 4, Color(206, 206, 206), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, Color(0, 0, 0))
				...
				if (#commands == 1) then
					...
					if (v.aliases) then
						...
						draw.SimpleTextOutlined(fl.lang:TranslateText(text), "flChatFont", 4, cY + offsetX, Color(240, 240, 240), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, Color(0, 0, 0))

						offsetX = offsetX + 20
					end

					draw.SimpleTextOutlined(fl.lang:TranslateText("#CMDDesc_Usage ".."/"..v.name.." "..v.syntax), "flChatFont", 4, cY + offsetX, Color(240, 240, 240), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, Color(0, 0, 0))
				end
...
vgui.Register("flChatBox", PANEL, "EditablePanel")
...
function PANEL:Paint(w, h)
	...
	self:DrawTextEntryText(fl.settings:GetColor("TextColor"), ColorAlpha(fl.settings:GetColor("TextColor"), 25), fl.settings:GetColor("TextColor"))
end
...
vgui.Register("flChatTextEntry", PANEL, "DTextEntry")
...
function chatbox.CreateChatBox()
	...
	chatbox.panel = vgui.Create("flChatBox")
end
...
function chatbox.CreateTextEntry()
	...
	chatbox.textEntry = vgui.Create("flChatTextEntry", chatbox.panel)
	chatbox.textEntry:SetFont("flChatFont")
...
concommand.Add("fl_resetchat", chatbox.RecreatePanel)
...
function chatbox.UpdateDisplay()
	...
	local i = 1
	local textW, textH = util.GetTextSize("asdf1234", "flChatFont")
