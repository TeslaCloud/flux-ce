...
function character.create(player, data)
  if (!isstring(data.name) or (data.utf8.len(name) < config.get('character_min_name_len') or
    data.utf8.len(name) > config.get('character_max_name_len'))) then
    return CHAR_ERR_NAME
  end

  if (!isstring(data.phys_desc) or (data.utf8.len(phys_desc) < config.get('character_min_desc_len') or
    data.utf8.len(phys_desc) > config.get('character_max_desc_len'))) then
