...
function PLUGIN:HUDPaint()
	if (IsValid(rw.client) and rw.client:Alive() and rw.client:IsOperator() and rw.client:GetMoveType() == MOVETYPE_NOCLIP) then
		local scrW, scrH = ScrW(), ScrH()

		for k, v in ipairs(_player.GetAll()) do
			if (v == rw.client) then continue end
			...
			local distance = rw.client:GetPos():Distance(v:GetPos())
