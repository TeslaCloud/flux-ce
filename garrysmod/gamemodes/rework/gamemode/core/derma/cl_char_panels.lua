...
vgui.Register("rwCharCreationBase", PANEL, "rwBasePanel")
...
vgui.Register("rwCharCreationGeneral", PANEL, "rwCharCreationBase")
...
function PANEL:Rebuild()
	...
	self.scrollpanel = vgui.Create("rwSidebar", self)
...
vgui.Register("rwCharCreationModel", PANEL, "rwCharCreationBase")
...
function PANEL:Init()
	...
	self.Chooser = vgui.Create("rwSidebar", self)
	self.Chooser:SetPos(32, 90)
	self.Chooser:SetSize(500, ScrH() - 290)
	self.Chooser:AddSpace(2)

	for k, v in pairs(faction.GetAll()) do
		if (!v.Whitelisted or rw.client:HasWhitelist(v.uniqueID)) then
			local button = vgui.Create("rwImageButton", self)
...
vgui.Register("rwCharCreationFaction", PANEL, "rwCharCreationBase");