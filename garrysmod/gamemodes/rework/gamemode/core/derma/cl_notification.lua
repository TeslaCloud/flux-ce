...
function PANEL:SizeToContents()
	...
	for k, v in ipairs(self.notificationText) do
		local w, h = util.GetTextSize(v, "rw_menuitem");
...
function PANEL:Paint(width, height)
	...
	if (!theme.Hook("PaintNotificationText", self, width, height)) then
		...
		for k, v in ipairs(self.notificationText) do
			draw.SimpleText(v, "rw_menuitem", 4, curY, ColorAlpha(self.textColor, self.curAlpha + 40));
...
vgui.Register("reNotification", PANEL, "EditablePanel");